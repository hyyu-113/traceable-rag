...
根据已索引的差旅制度回答住宿标准问题，同时展示正文和表格中的依据，支持下载原文核对。

![问答与原文核对](docs/images/answer-citations.png)

### 文档导入与状态展示

支持 PDF、Word 和 Excel 文档导入，展示索引状态及证据块数量。截图同时保留了成功与失败记录；导入失败时，页面提供重新上传入口。

![文档导入与状态展示](docs/images/documents.png)
...
默认使用 **pdfminer.six** 提取 PDF 文字、页码和引用位置。当前功能以文字检索为主，它能提供所需信息，MIT 许可也便于后续部署和交付。

需要页面渲染或评估批量解析性能时，可考虑接入 PyMuPDF。它采用 AGPL / 商业双许可，需要满足相应授权条件，因此未作为默认依赖。替换方式及当前解析限制见 [文档解析选型](docs/dependencies.md)。
